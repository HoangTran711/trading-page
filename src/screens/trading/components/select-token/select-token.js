...
const onRenderCell = (
	...
	return (
		<div
			onClick={() => onHandleSelectToken(item)}
			className={`${
				dataContext.tokenSell?.Symbol === item.Symbol ||
				dataContext.tokenReceive?.Symbol === item.Symbol
					? 'opacity-50'
					: ''
			} cursor-pointer ${classNames.itemCell}`}
			data-is-focusable={true}
		>
			<Persona
				{...tokensPersona}
				presence={
					item.Verified ? PersonaPresence.online : PersonaPresence.offline
				}
				imageAlt='Image'
			/>
		</div>
...
export const SelectToken = () => {
	...
	const { data: searchIndex, isSuccess } = useSearchableOnlyVerifiedToken(
		'Symbol',
		'PSymbol',
		'Name',
		'TokenID'
	...
	const tokenList = React.useMemo(() => {
		if (isSuccess) {
			...
			return orderBy(
				filter(data.tokens, { Verified: true }),
				['Verified', 'Symbol', 'PSymbol', 'IsCustom'],
				['desc', 'asc', 'asc', 'desc']
			)
		}
		return []
		...
		<div className='select-token'>
			...
			<div className='select-token-panel'>
				<div className='heading-primary'>
					<h3>Select Token</h3>
					<IconButton
						onClick={data.onHandleSelectTokens}
				...
				<div className='input-token'>
					<input
						onChange={(e) => data.setValueInputSearch(e.target.value)}
						value={data.valueInputSearch}
						type='text'
				...
				<div className='list-token'>
					...
					{data.fetchTokensSuccess ? (
						<ListTokens
							dataContext={data}
							onHandleSelectToken={onHandleSelectToken}
							tokens={onCheckCalculate(tokenList)}
						/>
					) : null}
				</div>
			</div>
