...
export const OverlayDetail = ({ impact, fee, poolSize }) => {
	...
	const onFixedNumber = (fee) => {
		if (fee.fee < 100) {
			return (fee?.fee * Math.pow(10, -fee.feeToken?.pDecimals)).toFixed(
				fee.feeToken?.pDecimals - 1
			)
		} else if (fee.fee >= 100 && fee.fee < 1000) {
			return (fee?.fee * Math.pow(10, -fee.feeToken?.pDecimals)).toFixed(
				fee.feeToken?.pDecimals - 2
			)
		} else {
			return (fee?.fee * Math.pow(10, -fee.feeToken?.pDecimals)).toFixed(
				fee.feeToken?.pDecimals - 3
			)
		}
	}
	return (
		<div className='overlay-detail'>
			...
			<div className='item first-item'>
				...
				<span className=''>
					{data.tokenSell?.id && data.tokenReceive?.id
						? onFixedNumber(fee) || 0
						: 0}
				</span>
			</div>
			{poolSize.output1 ? (
				poolSize.output2 ? (
					...
					<div className='item first-item'>
						<span className='label'>Pool Size</span>
						<span className='pool-size'>{`${poolSize?.output1[0]} ${
							data.tokenSell.id === PRV_ID
								? data.tokenReceive.symbol
								: data.tokenSell.symbol
						} + ${poolSize?.output1[1]} ${PRV.symbol}`}</span>
					</div>
				)
			) : (
				<div className='item first-item'>
