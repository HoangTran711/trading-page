...
import { faTimes, faArrowUp, faThumbsUp, faExternalLinkAlt, faThumbsDown } from '@fortawesome/free-solid-svg-icons'
import './popup.css'
...
export const PopupSuccess = ({title,content}) => {
	...
	const onHandleViewDetail = () => {
		if(title === 'Trade request') {
			window.open(`https://mainnet.incognito.org/tx/${data.linkDetail}`, '_blank')
		}
	}
	return (
		<div className="popup">
			...
			<div className="txt-frame">
				<div className="view-on">
					<FontAwesomeIcon className="icon" icon={title !== 'Connect' ? faArrowUp : faThumbsUp} />
				</div>
				<div onClick={onHandleViewDetail} className={`content ${title === 'Connect' ? 'connect' : ''}`} >
					<span>{content}</span>
					{title !== 'Connect' ? <FontAwesomeIcon icon={faExternalLinkAlt} className="icon" /> : null}
				</div>
				<div className="btn-pop-container">
