...
export const calculateInputValueCrossPool = (
	pairs,
	inputToken,
	outputValue,
	outputToken
	...
	if (secondPair) {
		...
		inputValue = calculateInputValue(
			firstPair,
			inputToken,
			inputValue,
			currentOutputToken
...
export const calculateSizeImpact = (
	...
	const totalInputUsd = toHumanAmount(
		inputValue * (inputPrice?.priceUsd || inputPriceUsd),
		inputPDecimals
	)
	const totalOutputUsd = toHumanAmount(
		outputValue * (outputPrice?.priceUsd || outputPriceUsd),
		outputPDecimals
	)
...
export const getPoolSize = (inputToken, outputToken, pairs) => {
	...
	if (firstPair) {
		...
		const formattedInputPool = amount(
			inputPool,
			inputToken.id === PRV_TOKEN_ID
				? outputToken?.pDecimals
				: inputToken?.pDecimals,
			true
		)
		const formattedOutputPool = amount(outputPool, PRV.pDecimals, true)
		if (secondPair) {
			...
			const formattedOutputPool2 = amount(outputPool2, PRV.pDecimals, true)
			return {
...
export const onRoundBalance = (balance, pDecimals) => {
	if (balance && pDecimals) {
		return (parseInt(balance) * Math.pow(10, -pDecimals)).toFixed(2)
	}
	return '0.00'
}
