...
export async function getKyberQuote({ sellToken, sellAmount, buyToken }) {
	...
	const bestRate = rates?.data.Result.ListRate?.find(
		(rate) => rate?.DappName === rates?.data.Result.DappName
	)
	let expectAmount
	let protocol
	let dAppAddress
	...
	if (bestRate) {
		try {
			maxAmountIn = new BigNumber(
				bestRate?.AmountInput || rates?.AmountInput || 0
			)
				.dividedToIntegerBy(1)
				.toNumber()
			maxAmountOut = new BigNumber(
			...
			priorityArray.forEach((priority, index) => {
				...
				const tradingFee =
					new BigNumber(priority?.PRVAmount || 0)
						.dividedToIntegerBy(1)
						.toNumber() || 0
