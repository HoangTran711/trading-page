...
export const getPrivacyTokens = () => {
	return fetch(
		'https://api-service.incognito.org/pcustomtoken/list?tokenId=undefined'
	)
		.then((res) => res.json())
		.then((result) => {
...
export const getPairsData = async () => {
	try {
		const now = Date.now()
		const [pTokens, chainTokens, chainPairs] = await Promise.all([
			getTokenList(),
			getPrivacyTokens(),
			getPDEState(),
		])
		...
		const pairs = _(chainPairs.PDEPoolPairs)
			.map((pair) => ({
				...
				total:
					toRealTokenValue(tokens, pair.Token1IDStr, pair.Token1PoolValue) +
					toRealTokenValue(tokens, pair.Token2IDStr, pair.Token2PoolValue),
